...
class Storage {
    ...
    ~Storage();
    // CRUD for User & Meeting using C++11 Function
    // Template and Lambda Expressions
    void createUser(const User&);
    std::list<User> queryUser(std::function
            <bool(const User&)> filter);  // return found users
    ...
    std::list<Meeting> queryMeeting(std::function
            <bool(const Meeting&)> filter);  // return found meetings
    ...
    Storage();
    // storage structure with list,
    // or you have better structures e.g. balanced tree
